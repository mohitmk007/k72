...
import FullBodyNav from './FullBodyNav';
...
const Navbar = () => {
    ...
    return (
        <div>
            {isOpen === true ? <div className='z-50 '>
                <FullBodyNav />
            </div> :

                <div className='w-full fixed top-0 z-40 flex  justify-between'>
                    <div className="w-28 m-2 ">
                        <svg className='w-full svgLogo' fill='white' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103 44">
                    ...
                    }} className='h-14 w-60 bg-black  relative '>
                        <div ref={navGreenRef} className='h-0 w-full bg-[#d3fd50] absolute top-0 transition-all'></div>
                        <div onClick={() => setIsOpen(!isOpen)} className='relative w-full h-full cursor-pointer flex items-center justify-end pr-10 text-4xl text-white hover:text-black'>
                            <RiMenu3Fill />
                        </div>
